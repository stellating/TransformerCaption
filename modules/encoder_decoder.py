...
class EncoderDecoder(AttModel):
    ...
    def __init__(self, args, tokenizer):
        ...
        self.embeded = Embeddings(2048, tgt_vocab)
        self.model = self.make_model(tgt_vocab)
        self.logit = nn.Linear(args.d_model, tgt_vocab)
        self.logit_mesh = nn.linear(args.d_model, )

    def init_hidden(self, bsz):
        return []

    def _prepare_feature(self, fc_feats, att_feats, att_masks, meshes):

        att_feats, seq, att_masks, seq_mask = self._prepare_feature_forward(att_feats, att_masks, meshes)
    ...
    def _prepare_feature_forward(self, att_feats, att_masks=None, meshes=None, seq=None):
        ...
        att_feats = self.multimodal_feat(att_feats,mesh)
        att_feats, att_masks = self.clip_att(att_feats, att_masks)
        att_feats = pack_wrapper(self.att_embed, att_feats, att_masks)
    ...
    def _forward(self, fc_feats, att_feats, meshes, seq, att_masks=None):
        cnn_feats = att_feats
        att_feats, mesh, att_masks, mesh_mask = self._prepare_feature_mesh(att_feats, att_masks, meshes)
        out_mesh = self.model(att_feats,mesh, att_masks, mesh_mask)
        outputs = F.log_softmax(self.logit_mesh(out_mesh), dim=-1)

        att_feats, seq, mesh, att_masks, seq_mask = self._prepare_feature_forward(att_feats, att_masks, meshes, seq)
